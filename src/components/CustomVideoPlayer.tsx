...
import { Play, Pause, RotateCcw, Palette, Maximize, CornerDownLeft, Crop, Upload, Image as ImageIcon, Settings, ZoomIn, Plus, Download, FileVideo } from 'lucide-react';
...
import { useVideoProcessor, VideoExportOptions } from '@/hooks/useVideoProcessor';
...
export const CustomVideoPlayer: React.FC<CustomVideoPlayerProps> = ({ 
  ...
  const checkBrowserCompatibility = useCallback(() => {
    ...
    if (window.location.protocol !== 'https:' && window.location.hostname !== 'localhost') {
      issues.push('HTTPS required for FFmpeg WebAssembly (current: ' + window.location.protocol + ')');
    }
    
    if (typeof SharedArrayBuffer === 'undefined') {
      issues.push('SharedArrayBuffer not supported - this is critical for FFmpeg');
    }
    
    if (!window.crossOriginIsolated) {
      warnings.push('Cross-origin isolation not enabled - may cause issues');
    }
    
    if (!('WebAssembly' in window)) {
      issues.push('WebAssembly not supported - required for FFmpeg');
    }
  ...
  // Test FFmpeg loading function
  const testFFmpegLoading = async () => {
    try {
      const { issues, warnings } = checkBrowserCompatibility();
      
      if (issues.length > 0) {
        console.error('🚨 Critical browser compatibility issues:', issues);
        toast.error(`Cannot load FFmpeg: ${issues.join(', ')}`, { duration: 10000 });
        return;
      }
      
      if (warnings.length > 0) {
        console.warn('⚠️ Browser compatibility warnings:', warnings);
        toast.warning(`Browser warnings: ${warnings.join(', ')}`, { duration: 8000 });
      }
      
      toast.info('Testing FFmpeg loading...', { duration: 2000 });
      ...
      await loadFFmpeg();
      toast.success('FFmpeg loaded successfully! Export should work.', { duration: 5000 });
      ...
      if (errorMessage.includes('HTTPS') || errorMessage.includes('secure')) {
        toast.error('FFmpeg requires HTTPS. Please access the site via https:// or use localhost for development.', { duration: 10000 });
      } else if (errorMessage.includes('SharedArrayBuffer')) {
        toast.error('Your browser does not support SharedArrayBuffer. Please use Chrome 88+ or Firefox 89+.', { duration: 10000 });
      } else {
        toast.error(`FFmpeg test failed: ${errorMessage}`, { duration: 8000 });
      }
    }
  };

  // Show compatibility warnings on component mount
  useEffect(() => {
    const { issues, warnings } = checkBrowserCompatibility();
    
    if (issues.length > 0) {
      console.error('🚨 Critical browser compatibility issues detected:', issues);
      // Show a warning toast about the issues
      setTimeout(() => {
        toast.warning('Video export may not work: ' + issues[0], { duration: 8000 });
      }, 2000);
    } else if (warnings.length > 0) {
      console.warn('⚠️ Browser compatibility warnings detected:', warnings);
    } else {
  ...
  const handleExport = async () => {
    if (!src) {
      toast.error('No video loaded for export');
      return;
    }
    ...
    try {
      ...
      toast.info('Starting video export...', { duration: 2000 });
      
      // Load FFmpeg first with progress feedback
      console.log('📚 Loading FFmpeg...');
      toast.info('Loading video processing engine...', { duration: 3000 });
      
      try {
        await loadFFmpeg();
        console.log('✅ FFmpeg loaded successfully');
        toast.success('Video processing engine loaded!', { duration: 2000 });
      } catch (ffmpegError) {
        console.error('FFmpeg loading failed:', ffmpegError);
        toast.error('Failed to load video processing engine. Please check your internet connection and try again.');
        return;
      }

      // Fetch the video blob
      console.log('📥 Fetching video data...');
      toast.info('Preparing video for processing...', { duration: 2000 });
      ...
      try {
        ...
        console.log(`📹 Video loaded: ${(videoBlob.size / 1024 / 1024).toFixed(1)}MB`);
      } catch (fetchError) {
      ...
      console.log('🔧 Export configuration:', exportOptions);
      toast.info('Processing video with all effects...', { duration: 3000 });

      // Process video with progress callback
      const exportedBlob = await processVideo(
        videoBlob, 
        exportOptions, 
        (progress) => {
          setExportProgress(progress);
          
          // Show toast updates at key milestones
          if (progress === 25) {
            toast.info('Applying video filters...', { duration: 2000 });
          } else if (progress === 50) {
            toast.info('Processing zoom effects...', { duration: 2000 });
          } else if (progress === 75) {
            toast.info('Finalizing export...', { duration: 2000 });
          }
        }
      );

      console.log(`✅ Export completed: ${(exportedBlob.size / 1024 / 1024).toFixed(1)}MB`);

      // Download the exported video
      const downloadUrl = URL.createObjectURL(exportedBlob);
      const a = document.createElement('a');
      a.href = downloadUrl;
      a.download = `edited-video-${new Date().toISOString().slice(0, 19).replace(/:/g, '-')}.${exportFormat}`;
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      URL.revokeObjectURL(downloadUrl);

      toast.success(`Video exported successfully as ${exportFormat.toUpperCase()}! (${(exportedBlob.size / 1024 / 1024).toFixed(1)}MB)`, { 
        duration: 5000 
      });
      ...
      if (errorMessage.includes('timeout')) {
        toast.error('Export timed out. Try reducing video length or quality settings.', { duration: 8000 });
      } else if (errorMessage.includes('memory')) {
        toast.error('Not enough memory. Try refreshing the page and reducing video complexity.', { duration: 8000 });
      } else if (errorMessage.includes('FFmpeg loading failed')) {
        toast.error('Failed to load video processor. Check your internet connection and try again.', { duration: 8000 });
      } else if (errorMessage.includes('Invalid video blob')) {
        toast.error('Invalid video file. Please try recording a new video.', { duration: 8000 });
      } else {
        toast.error(`Export failed: ${errorMessage}`, { duration: 8000 });
      }
      ...
      console.error('Export options:', {
        ...
        videoPadding,
        backgroundColor,
        zoomEffects: zoomEffects.length,
        exportQuality,
        exportFormat
      });
      console.groupEnd();
      ...
      setTimeout(() => {
        console.log('🧹 Export process cleanup completed');
      }, 1000);
    }
  };
      ...
      <div className="flex items-center justify-between p-4 bg-card border-t">
        ...
        <div className="flex items-center gap-4">
          ...
          >
            {isExporting ? (
              <>
                <FileVideo className="h-4 w-4 animate-spin" />
                Exporting...
              </>
            ) : (
              <>
        ...
        <div className="p-4 bg-card border-t">
          <div className="space-y-2">
            <div className="flex items-center justify-between text-sm">
              <span>Exporting video with all edits...</span>
              <span>{Math.round(exportProgress)}%</span>
            </div>
            <Progress value={exportProgress} className="w-full" />
            <div className="text-xs text-muted-foreground">
              Processing: Trimming, cropping, zoom effects, backgrounds, and quality settings
            </div>
          </div>
        </div>
      )}

      {/* Diagnostic Information (Development) */}
      {process.env.NODE_ENV === 'development' && (
        <div className="p-4 bg-muted/50 border-t border-dashed">
          <div className="space-y-2">
            <h4 className="text-sm font-medium">🔧 Export Diagnostics</h4>
            <div className="grid grid-cols-2 gap-2 text-xs">
              <div>
                <span className="font-medium">Protocol:</span> {window.location.protocol}
              </div>
              <div>
                <span className="font-medium">Secure Context:</span> {window.isSecureContext ? '✅' : '❌'}
              </div>
              <div>
                <span className="font-medium">SharedArrayBuffer:</span> {typeof SharedArrayBuffer !== 'undefined' ? '✅' : '❌'}
              </div>
              <div>
                <span className="font-medium">Cross-Origin Isolated:</span> {window.crossOriginIsolated ? '✅' : '⚠️'}
              </div>
              <div>
                <span className="font-medium">WebAssembly:</span> {'WebAssembly' in window ? '✅' : '❌'}
              </div>
              <div>
                <span className="font-medium">User Agent:</span> {navigator.userAgent.includes('Chrome') ? 'Chrome' : navigator.userAgent.includes('Firefox') ? 'Firefox' : 'Other'}
              </div>
            </div>
            <div className="text-xs text-muted-foreground mt-2">
              📍 For export to work: Need HTTPS + SharedArrayBuffer + WebAssembly
            </div>
          </div>
        </div>
      )}
